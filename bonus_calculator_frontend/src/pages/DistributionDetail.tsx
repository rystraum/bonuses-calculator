...
function HandoffButton({ dist }: { dist: Distribution }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline"><Share2 className="mr-1.5 h-4 w-4" /> Handoff view</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Handoff — {dist.name}</DialogTitle>
        </DialogHeader>
        {result ? (
              ...
              <table className="ledger">
                ...
                <tbody>
                  {result.payouts.map((p) => {
                    // Aggregate across all bonus groups and special bonuses —
                    // no impact/effort or per-group breakdown in the handoff.
