...
export default function Handoff() {
  ...
  const { refreshDistribution, loadApprovals } = store
  ...
  // Fresh snapshot + computation (+ approvals) whenever this page is opened directly.
  useEffect(() => {
    if (!id) return
    refreshDistribution(id).catch(() => {})
    loadApprovals(id).catch(() => {})
  }, [refreshDistribution, loadApprovals, id])
  ...
  const approvalsLine = approvals
    .map((a) => `${a.user.username} on ${fmtDateTime(a.approvedAt)}`)
    .join('  ·  ')
    ...
    <div className="mb-2 flex flex-wrap items-end justify-between gap-x-6 gap-y-1 border-b pb-2">
      ...
      {approvalsLine && (
        <div className="col-span-2 mt-1 text-xs text-muted-foreground">
          Approved by <span className="font-medium text-foreground">{approvalsLine}</span>
        </div>
      )}
    </div>
  )
  ...
  const downloadPdf = () => {
    ...
    const drawHeader = () => {
      ...
      if (approvalsLine) doc.text(`Approved by ${approvalsLine}`, 14, 26)
    }

    let lastY = 28
    const tableBase = {
      margin: { left: 14, right: 14, top: 31 },
      theme: 'grid' as const,
  ...
  const downloadExcel = () => {
    ...
    const detailsSheet = XLSX.utils.aoa_to_sheet([
      ...
      ...approvals.map((a) => [`Approved by ${a.user.username}`, fmtDateTime(a.approvedAt)]),
    ])
    detailsSheet['!cols'] = [{ wch: 24 }, { wch: 44 }]
          ...
          <p className="border-b pb-3 text-xs text-muted-foreground">
            ...
            {approvals.map((a, i) => (
              <span key={a.id}>
                ...
                {' on '}
                <span className="font-medium text-foreground">{fmtDateTime(a.approvedAt)}</span>
              </span>
            ))}
          </p>
        )}
